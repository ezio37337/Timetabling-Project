...
# hard cons
# Check if the same course is scheduled in different rooms in the same time slot 
def time_violated(timetable):
...
def soft_room_usage_balance(timetable):
...
def CalculateFitness(timetable):
    ...
    if room_violated(timetable):
        fitness -= highPenalty
    
    if time_violated(timetable):
        fitness -= highPenalty

    if course_violated(timetable):
        fitness -= highPenalty

    # soft 
    if soft_breaks(timetable):
        fitness -= lowPenalty

    if soft_room_usage_balance(timetable):
        fitness -= soft_room_usage_balance(timetable)
