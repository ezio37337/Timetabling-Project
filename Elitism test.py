...
def genetic_algorithm(units, teachers, days, time_slots, rooms, students, df_rooms, ELITISM_RATE):
    num_units = len(units)
    num_days = len(days)
    num_time_slots = len(time_slots)
...
if __name__ == '__main__':
    # Define ELITISM_RATE range
    elitism_rates = np.arange(0.05, 0.201, 0.02)

    # Lists to store results
    elitism_rate_list = []
    average_fitness_list = []

    # For each ELITISM_RATE value
    for ELITISM_RATE in elitism_rates:
        fitness_values = []
        for run in range(5):  # Run 5 times for each ELITISM_RATE
            ...
            (best_schedule, best_fitness, hard_constraints, soft_constraints,
             initial_schedule_df, initial_time_slot_violations, best_time_slot_violations, fitness_history) = genetic_algorithm(
                units, teachers, days, time_slots, rooms, students, df_rooms, ELITISM_RATE)
            end_time = time.time()

            print(f"ELITISM_RATE: {ELITISM_RATE:.2f}, Run {run + 1}/5 completed in {end_time - start_time:.2f} seconds")
        ...
        # Compute average fitness for current ELITISM_RATE
        average_fitness = np.mean(fitness_values)
        elitism_rate_list.append(ELITISM_RATE)
        average_fitness_list.append(average_fitness)

    # Plot ELITISM_RATE vs Average Fitness
    plt.figure(figsize=(10, 6))
    plt.plot(elitism_rate_list, average_fitness_list, marker='o')
    plt.title('Elitism Rate vs Average Best Fitness')
