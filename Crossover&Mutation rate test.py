...
if __name__ == '__main__':
    # 定义交叉率和变异率的范围
    crossover_rates = np.arange(0.4, 0.91, 0.02)
    mutation_rates = np.arange(0.25, 0.0, -0.01)
    ...
    total_runs = len(crossover_rates) * len(mutation_rates) * 5
    current_run = 0

    for CROSSOVER_RATE in crossover_rates:
        for MUTATION_RATE in mutation_rates:
            for run in range(10):  # 每个参数组合运行5次
                ...
                # 为绘图保存数据
                crossover_rate_all.append(CROSSOVER_RATE)
                mutation_rate_all.append(MUTATION_RATE)
                fitness_values_all.append(best_fitness)

                current_run += 1
                print(f"Run {current_run}/{total_runs}: Crossover Rate: {CROSSOVER_RATE:.2f}, Mutation Rate: {MUTATION_RATE:.2f}, Best Fitness: {best_fitness}")

    # 将结果保存到 CSV 文件
    df_results = pd.DataFrame(results)
    df_results.to_csv('crossover_mutation_results.csv', index=False)

    # 准备数据用于 3D 表面图
    X = np.array(crossover_rate_all)
    Y = np.array(mutation_rate_all)
    Z = np.array(fitness_values_all)

    # 创建三角化
    fig = plt.figure(figsize=(12, 8))
    ax = fig.add_subplot(111, projection='3d')

    # 使用 plot_trisurf 绘制 3D 表面图
    surf = ax.plot_trisurf(X, Y, Z, cmap='viridis', edgecolor='none')

    ax.set_xlabel('Crossover Rate')
    ax.set_ylabel('Mutation Rate')
    ax.set_zlabel('Best Fitness')
    ax.set_title('Crossover Rate vs Mutation Rate vs Best Fitness')

    fig.colorbar(surf, ax=ax, shrink=0.5, aspect=5)

    plt.savefig('crossover_mutation_fitness_3d_surface.png')
    plt.show()
