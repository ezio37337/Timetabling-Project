...
CROSSOVER_RATE = 0.9
ELITISM_RATE = 0.1
PATIENCE = GENERATIONS + 10  # 防止提前停止，确保每次运行都有相同的代数
...
def initialize_population(units, teachers, days, time_slots, df_rooms, rooms, room_types, students, POPULATION_SIZE, GREEDY_PERCENTAGE):
    population = []
    num_greedy = int(POPULATION_SIZE * GREEDY_PERCENTAGE)
...
def genetic_algorithm(units, teachers, days, time_slots, rooms, students, df_rooms, GREEDY_PERCENTAGE):
    num_units = len(units)
    num_days = len(days)
    num_time_slots = len(time_slots)
    ...
    population = initialize_population(units, teachers, days, time_slots, df_rooms, rooms, room_types, students, POPULATION_SIZE, GREEDY_PERCENTAGE)
    ...
    return (
        ...
        initial_schedule_df, {}, best_time_slot_violations, fitness_history
    )
...
if __name__ == '__main__':
    # Define GREEDY_PERCENTAGE range
    greedy_percentages = np.arange(0.1, 0.61, 0.1)

    # Lists to store results
    greedy_percentage_list = []
    average_fitness_list = []

    # For each GREEDY_PERCENTAGE value
    for GREEDY_PERCENTAGE in greedy_percentages:
        fitness_values = []
        for run in range(5):  # Run 5 times for each GREEDY_PERCENTAGE
            start_time = time.time()
            (best_schedule, best_fitness, hard_constraints, soft_constraints,
             initial_schedule_df, initial_time_slot_violations, best_time_slot_violations, fitness_history) = genetic_algorithm(
                units, teachers, days, time_slots, rooms, students, df_rooms, GREEDY_PERCENTAGE)
            end_time = time.time()

            print(f"GREEDY_PERCENTAGE: {GREEDY_PERCENTAGE:.2f}, Run {run + 1}/5 completed in {end_time - start_time:.2f} seconds")
            print(f"Best Fitness: {best_fitness}, Hard Constraints Violations: {hard_constraints}, Soft Constraints Violations: {soft_constraints}")

            fitness_values.append(best_fitness)

        # Compute average fitness for current GREEDY_PERCENTAGE
        average_fitness = np.mean(fitness_values)
        greedy_percentage_list.append(GREEDY_PERCENTAGE)
        average_fitness_list.append(average_fitness)

    # Plot GREEDY_PERCENTAGE vs Average Fitness
    plt.figure(figsize=(10, 6))
    plt.plot(greedy_percentage_list, average_fitness_list, marker='o')
    plt.title('Greedy Initialization Percentage vs Average Best Fitness')
    plt.xlabel('Greedy Initialization Percentage')
