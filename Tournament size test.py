...
teachers = df_units['Teacher'].fillna('').tolist()  # Replace NaN with empty string
...
CROSSOVER_RATE = 0.9
ELITISM_RATE = 0.1
PATIENCE = GENERATIONS + 10  # Prevent early stopping
    ...
    for i in units_sorted:
        ...
        for day_idx in range(num_days):
            for time_slot_idx in range(num_time_slots - (duration - 1)):
                ...
                if isinstance(teacher, str) and teacher != 'None' and teacher != '':
...
def fitness(chromosome, units, teachers, days, time_slots, rooms, students,
    ...
    HARD_CONSTRAINT_PENALTY = 500  # Increase hard constraint penalty
...
# Tournament selection function with variable tournament size
...
def genetic_algorithm(units, teachers, days, time_slots, rooms, students, df_rooms, tournament_size):
    num_units = len(units)
    num_days = len(days)
    num_time_slots = len(time_slots)
...
if __name__ == '__main__':
    # Define tournament size range
    tournament_sizes = range(2, 11)  # From 2 to 10 inclusive

    # Lists to store results
    tournament_size_list = []
    average_fitness_list = []

    # For each tournament size value
    for tournament_size in tournament_sizes:
        fitness_values = []
        for run in range(10):  # Run 2 times for each tournament size
            ...
            (best_schedule, best_fitness, hard_constraints, soft_constraints,
             initial_schedule_df, initial_time_slot_violations, best_time_slot_violations, fitness_history) = genetic_algorithm(
                units, teachers, days, time_slots, rooms, students, df_rooms, tournament_size)
            end_time = time.time()

            print(f"Tournament Size: {tournament_size}, Run {run + 1}/10 completed in {end_time - start_time:.2f} seconds")
        ...
        # Compute average fitness for current tournament size
        average_fitness = np.mean(fitness_values)
        tournament_size_list.append(tournament_size)
        average_fitness_list.append(average_fitness)

    # Plot Tournament Size vs Average Fitness
    plt.figure(figsize=(10, 6))
    plt.plot(tournament_size_list, average_fitness_list, marker='o')
    plt.title('Tournament Size vs Average Best Fitness')
