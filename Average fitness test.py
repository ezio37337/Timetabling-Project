...
def genetic_algorithm(units, teachers, days, time_slots, rooms, students, df_rooms):
    ...
    # Remove plotting from genetic_algorithm function
...
if __name__ == '__main__':
    # List to store fitness histories of all runs
    fitness_histories = []
    ...
    for run in range(30):
        ...
        print(f"Run {run + 1}/30 completed in {end_time - start_time:.2f} seconds")
    ...
    # Convert fitness histories to numpy array for easier calculation
    fitness_histories_array = np.array(fitness_histories)

    # Compute average fitness at each generation
    average_fitness = np.mean(fitness_histories_array, axis=0)

    # Plot average fitness over generations
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(average_fitness) + 1), average_fitness, label='Average Fitness')
